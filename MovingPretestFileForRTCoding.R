...
for (i in 1:length(A)){
  ...
  new.finaltest <- paste("//cnas.ru.nl/wrkgrp/STD-EXP_5_Katya/BACK-UP/", pNumber,"/",pNumber,"_Finaltest/", sep="")
  new.folder.org <- paste("//cnas.ru.nl/wrkgrp/STD-EXP_5_Katya/CODING/English_Pretest/", pNumber,"_Pretest_subset/", sep="")
  ...
  if (dir.exists(new.folder.org)){}else{
    ...
    setwd(new.parent)
    file.copy(infile1, new.folder.org, overwrite = T, recursive = T, copy.mode = T)
